"use client";

import { useState } from "react";
import { pdf } from "@react-pdf/renderer";
import { FileDown, Sheet } from "lucide-react";
import { Button } from "@/components/ui/button";
import { gerarCsv, baixarCsv } from "@/lib/csv";
import { formatarMinutos } from "@/lib/duracao";
import type { CabecalhoPdf } from "@/components/pdf/relatorio";
import { FrequenciaDocumento } from "../pdf/frequencia-documento";
import type { RelatorioFrequencia } from "../types";

export function ExportarFrequencia({
  relatorio,
  programa,
  fusoHorario,
}: {
  relatorio: RelatorioFrequencia;
  programa: CabecalhoPdf;
  fusoHorario: string;
}) {
  const [gerando, setGerando] = useState(false);
  const [erro, setErro] = useState<string | null>(null);

  function exportarCsv() {
    setErro(null);
    try {
      const csv = gerarCsv(
        [
          "Integrante",
          "Presenças",
          "Encontros realizados",
          "Justificadas",
          "Faltas",
          "Horas",
          "Frequência (%)",
        ],
        relatorio.pessoas.map((p) => [
          p.nomeCompleto,
          p.presentes,
          relatorio.totalEncontros,
          p.justificadas,
          p.ausentes,
          formatarMinutos(p.minutos),
          p.percentual,
        ]),
      );
      baixarCsv(`frequencia_${relatorio.mesISO}`, csv);
    } catch (e) {
      setErro(e instanceof Error ? e.message : "Não foi possível gerar o CSV.");
    }
  }

  async function exportarPdf() {
    setGerando(true);
    setErro(null);
    try {
      const blob = await pdf(
        <FrequenciaDocumento
          relatorio={relatorio}
          programa={programa}
          geradoEmISO={new Date().toISOString()}
          fusoHorario={fusoHorario}
        />,
      ).toBlob();

      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `frequencia_${relatorio.mesISO}.pdf`;
      link.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      setErro(e instanceof Error ? e.message : "Não foi possível gerar o PDF.");
    } finally {
      setGerando(false);
    }
  }

  if (relatorio.pessoas.length === 0) return null;

  return (
    <div className="space-y-1">
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secundario" onClick={exportarCsv}>
          <Sheet className="h-4 w-4" strokeWidth={1.75} />
          Exportar CSV
        </Button>
        <Button type="button" variant="secundario" onClick={exportarPdf} disabled={gerando}>
          <FileDown className="h-4 w-4" strokeWidth={1.75} />
          {gerando ? "Gerando…" : "Exportar PDF"}
        </Button>
      </div>
      {erro && <p className="text-sm text-alert">{erro}</p>}
    </div>
  );
}
